...
> **Engineering focus:** multi-agent orchestration · Text-to-SQL · deterministic analytics · hybrid RAG · self-correction · evaluation-driven development · production-oriented guardrails
...
### Почему детерминированная аналитика?

LLM не используется для численных вычислений. Тренды, топ-N, аномалии и проценты считает Python (`analytics.py`); LLM отвечает только за интерпретацию и естественно-языковую подачу. Цифры в ответе всегда приходят из БД или Python-расчётов — никогда из генерации.

### Почему self-correction?

SQL ломается из-за неоднозначности схемы или диалект-специфичного синтаксиса. Упавший запрос возвращается агенту вместе с ошибкой БД — агент переписывает SQL и повторяет в рамках ограниченного числа попыток (2 раунда). Без этого остаётся два плохих варианта: пустой ответ или молчаливая подмена данных.

### Почему гибридный поиск?

BM25-IDF покрывает точные термины и идентификаторы, векторный поиск — семантическую близость и кросс-языковые запросы. Сигналы объединяются взвешенной комбинацией (0.4 BM25 / 0.6 vector) — без обучения ранжирующей модели.
