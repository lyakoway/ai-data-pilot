...
<sub>Медианы 3 прогонов одного вопроса через полный цикл (план → БД → ответ) — латентность внешнего API варьируется между запусками. GLM-5.3-поколение отвечает быстрее, но SQL генерирует слабее; GLM-5.2 — единственная без промахов. Скрипт замера: `python scripts/latency_benchmark.py --models ... --runs 3`.</sub>
