...
Два специализированных агента — **Атлас** (Data Agent: Text-to-SQL, графики,
Excel) и **Док** (Knowledge Agent: RAG по документам и загрузкам) — плюс
двухуровневый авто-роутер.
...
простоя занимает ~1 мин. Векторная модель (fastembed) загружается при первом
поиске Дока (~10 с).</sub>
...
3. **Два агента, не один промпт** — Атлас — SQL, Док — документы.
...
- 🧭 **Двойной авто-роутинг** — по агенту (данные → Атлас, документы → Док) и
...
- 👩‍💻 **Док (Knowledge Agent)**
  ...
  - **Загрузки** — PDF, Word, Excel, CSV, TXT, MD. Excel — ещё и SQL-таблица для Атласа.
...
3. **Анализ загруженных Excel-выгрузок** — файл с данными перетаскивается
   в окно — и по нему можно задавать вопросы: Атлас строит SQL по автосхеме,
   Док ищет по содержимому, JOIN между файлами работает из коробки.
...
[React-дашборд] ──/api──▶ [FastAPI]
                              ├─ Роутер агентов: данные → Атлас, документы → Док
                              ├─ Атлас: схема → SQL → guard → аналитика → chart/xlsx
                              ...
                              └─ Док: гибридный RAG (BM25-IDF + векторы fastembed)
...
таблицы). CSV/Excel входят в **оба** конвейера: SQL-таблица для Атласа и
текстовые чанки для Дока. Пароли источников остаются на сервере и никогда
...
| RAG Дока + app.db                       | 20     | steps, sources, цитаты, feedback stats                          |
...
Headline-качество — **~85% нормализованной точности**: те же числа без учёта алиасов
...
self-correction Атласа, качество поиска, роутинг и источники. Изолированные
...
4. В образ встроен демо-PostgreSQL (база `shop`, пользователь `demo`,
   `127.0.0.1:5432`, только localhost) — пресет **Подключить PostgreSQL**
   работает из коробки.
