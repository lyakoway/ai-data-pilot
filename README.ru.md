...
занимает ~1 мин. Векторная модель (fastembed) грузится при первом поиске Ксюши (~10 с).</sub>
...
- 👤 **Олег (Data Agent)** — ReAct tool-calling, SSE-trace, self-correction
  (до двух раундов), детерминированная аналитика на Python.
- 👩‍💻 **Ксюша (Knowledge Agent)** — гибрид BM25 + векторов (fastembed).
...
<sub>Медианы 3 прогонов **одного** вопроса. GLM-4.6 остаётся default: оценка ~85%
снята на ней. GLM-5.2 быстрее на этом семпле. GLM-5.3-flash ещё быстрее, но
SQL слабее. Полный цикл **~16–30 с** — не дашборд за долю секунды.</sub>

## Известные ограничения

- Нет санитизации загруженных файлов Ксюши от prompt injection.
- Routing/retrieval golden set маленькие. pytest retrieval (n=8) даёт 1.0 на
  всех режимах и не дискриминирует поиск.
- Self-correction покрыт юнит-тестами, не измерен как headline-метрика.
